...
const fn gen_precompiles<RT: Runtime<BS>, BS: Blockstore>() -> [PrecompileFn<RT>; 14] {
    [
...
fn td<RT: Runtime<BS>, BS: Blockstore>(rt: &RT, input: &[u8]) -> PrecompileResult {
    todo!()
}


fn get_actor_code_cid<RT: Runtime<BS>, BS: Blockstore>(rt: &RT, input: &[u8]) -> PrecompileResult {
...
fn get_randomness<RT: Runtime<BS>, BS: Blockstore>(rt: &RT, input: &[u8]) -> PrecompileResult {
...
fn lookup_address<RT: Runtime<BS>, BS: Blockstore>(rt: &RT, input: &[u8]) -> PrecompileResult {
    let id = EthAddress(input[..20].try_into().unwrap()).as_id();
    if id.is_none() {
        return Err(PrecompileError::InvalidInput)
    }
